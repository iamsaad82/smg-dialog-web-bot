...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const blob = new Blob([fileData], { type: file.mimetype || 'application/pdf' });
    ...
    const backendUrl = `${getBackendUrl()}/api/v1/documents/upload/pdf`;
    ...
    console.error('Fehler beim Upload der PDF-Datei:', error);
    ...
    return res.status(500).json({ 
      error: 'Interner Serverfehler beim Hochladen der PDF-Datei',

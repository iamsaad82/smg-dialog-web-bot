...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const blob = new Blob([fileData], { type: file.mimetype || 'application/json' });
    formData.append('file', blob, file.originalFilename || 'document.json');
    
    // API-Key aus dem Request-Header extrahieren
    const apiKey = req.headers['x-api-key'] || '';
    
    // Anfrage an das Backend senden
    const backendUrl = `${getBackendUrl()}/api/v1/documents/upload/json`;
    ...
    console.error('Fehler beim Upload der JSON-Datei:', error);
    ...
    return res.status(500).json({ 
      error: 'Interner Serverfehler beim Hochladen der JSON-Datei',

...
import { BrandenburgKindergarten } from '../types';
import { Baby, Phone, Mail, Globe, MapPin, Clock } from 'lucide-react';

/**
 * Renderer für Kindergarteninformationen aus Brandenburg
 */
export const KindergartenRenderer: React.FC<TenantRendererProps> = ({ data, className }) => {
  // Cast zu spezifischem Typ
  const kindergarten = data.data as BrandenburgKindergarten;

  return (
    <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 space-y-3 w-full ${className || ''}`}>
      {/* Header mit Kindergartentitel */}
      ...
      <div className="grid grid-cols-1 gap-3">
        ...
        {kindergarten.contact?.email && (
          <InfoRow 
            ...
            label="E-Mail" 
            value={kindergarten.contact.email} 
            type="email" 
          />
        )}
        
        {/* Website */}
        {(kindergarten.contact?.website || kindergarten.link) && (
          <InfoRow 
            icon={<Globe className="h-4 w-4 text-gray-500 dark:text-gray-400" />}
            label="Website" 
            value={kindergarten.contact?.website || kindergarten.link || ''} 
            type="website" 
          />
        )}
      </div>
